...
import {getTvByID } from "../APIS/themovieDB"
...
const TvDetail = () => {
    ...
    const {data, isLoading} = useQuery({
        ...
        select:(data) => (data as tvDetails),
        queryFn:() => getTvByID(Number.parseInt(id as string))
    })
    return <>
     {isLoading && (
        <Loading></Loading>
      ...
      {!isLoading && (<>
        <Helmet>
            <title>{import.meta.env.VITE_PROJECT_NAME + ` - ${data?.name}`}</title>
            ...
            <meta property="og:title" content={import.meta.env.VITE_PROJECT_NAME + ` - ${data?.name}`} />
        ...
        <div className="w-full h-full relative flex items-center justify-center">
            ...
            <div className="w-full md:w-3/5 h-full absolute z-10 flex flex-col items-center overflow-y-auto">
                ...
                <p className="font-Dmsans px-4 font-bold text-center text-slate-50">{data?.name}</p>
                <p className="font-Dmsans px-4 font-bold my-2 text-center text-slate-100">({data?.original_name})</p>
                <div className="w-full flex items-center justify-center flex-wrap">
                    {data?.genres.map((genre,index) => (
                        <div key={index} className="font-Dmsans font-semibold text-white-100 rounded-md  m-2 bg-cyan-800 p-1"><i className="bi bi-bookmark"></i> {genre.name}</div>
                    ))}
                </div>
                <div className="w-full flex items-center justify-center flex-wrap">
                    ...
                    <span className="font-Dmsans text-white-100 bg-bunker-950 p-2 m-2  rounded-md"><i className="bi bi-calendar-check"></i> {data?.last_air_date}</span>
                ...
                <p className="font-Dmsans font-medium text-white-100 p-4 md:px-40 text-justify ">
                        {data?.overview}
                </p>
